...
// ฟังก์ชันดึงข้อมูลสรุปของกลุ่มจาก LINE Messaging API
const getGroupSummary = async (groupId: string) => {
  try {
    const response = await axios.get(`https://api.line.me/v2/bot/group/${groupId}/summary`, {
      headers: {
        Authorization: `Bearer ${process.env.LINE_CHANNEL_ACCESS_TOKEN}`  // ตรวจสอบให้แน่ใจว่า LINE_CHANNEL_ACCESS_TOKEN ถูกตั้งค่าใน .env
      }
    });
    return response.data;  // response.data จะมีข้อมูล เช่น groupName, pictureUrl เป็นต้น
  } catch (error) {
...
const getGroupLine = async (groupId: string) => {
	...
	if(response.data?.data){
		console.log("Group data retrieved:", response.data.data);
		return response.data.data;
	}else{
		console.log("Group data not found for groupId:", groupId);
		return null;
	}
}

// ปรับปรุงฟังก์ชัน addGroupLine ให้รับ groupName เป็นพารามิเตอร์
const addGroupLine = async (groupId: string, groupName: string) => {
	console.log("Adding new group data for groupId:", groupId, "with groupName:", groupName);
	const response = await axios.post(`${process.env.WEB_DOMAIN}/api/master/getGroupLine`, { 
      group_line_id: groupId, 
      group_name: groupName 
  });
	if(response.data?.id){
		console.log("New group added with id:", response.data.id);
		return response.data.id;
	}else{
		console.log("Failed to add new group for groupId:", groupId);
		return null;
	}
}

const getUserTakecareperson = async (userId: string) => {
	console.log("Fetching user takecare person data for userId:", userId);
	const responseUser = await axios.get(`${process.env.WEB_DOMAIN}/api/user/getUserTakecareperson/${userId}`);
	if(responseUser.data?.data){
		console.log("User takecare person data retrieved:", responseUser.data.data);
		return responseUser.data.data;
	}else{
		console.log("User takecare person data not found for userId:", userId);
		return null;
	}
}

const getSafezone = async (takecare_id: number, users_id: number) => {
	console.log(`Fetching safezone data for takecare_id: ${takecare_id}, users_id: ${users_id}`);
	const response = await axios.get(`${process.env.WEB_DOMAIN}/api/setting/getSafezone?takecare_id=${takecare_id}&users_id=${users_id}`);
	if(response.data?.data){
		console.log("Safezone data retrieved:", response.data.data);
		return response.data.data;
	}else{
		console.log("Safezone data not found for takecare_id:", takecare_id, "users_id:", users_id);
		return null;
	}
}

const getLocation = async (takecare_id: number, users_id: number, safezone_id:number) => {
	console.log(`Fetching location data for takecare_id: ${takecare_id}, users_id: ${users_id}, safezone_id: ${safezone_id}`);
	...
	if(response.data?.data){
		...
		console.log("Location data not found for takecare_id:", takecare_id, "users_id:", users_id, "safezone_id:", safezone_id);
		return null;
	}
}
			...
			if(req.body.events[0]){
				...
				if(events.type === "message" && events.source.type === "user"){
					if(events.message.type === "text"){
						console.log("Received text message:", events.message.text);
				...
				// ส่วนจัดการ event เมื่อ bot เข้าร่วมกลุ่ม (join)
				if(events.source.type === "group" && events.type === "join"){
					...
					if(!groupLine){
						console.log("Group not found in DB, fetching group summary from LINE API...");
            const groupSummary = await getGroupSummary(events.source.groupId);
            let groupName = "";
            if(groupSummary && groupSummary.groupName){
              groupName = groupSummary.groupName;
            } else {
              groupName = "Group " + events.source.groupId;
            }
						console.log("Adding new group with name:", groupName);
						await addGroupLine(events.source.groupId, groupName);
					}
				}

				// ส่วนจัดการ event แบบ postback
